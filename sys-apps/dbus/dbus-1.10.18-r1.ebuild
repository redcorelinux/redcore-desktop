...
pkg_postinst() {
	...
	# Redcore Linux tweak to move the dbus service
	# from default runlevel to boot runlevel
	if [ "$(rc-config list default | grep dbus)" != "" ]; then
		ewarn "dbus is currently started from default runlevel"
		ewarn "moving it to boot runlevel"
		"${ROOT}"/sbin/rc-update del dbus default
		"${ROOT}"/sbin/rc-update add dbus boot
	elif [ "$(rc-config list boot | grep dbus)" != "" ]; then
		ewarn "dbus is currently started from boot runlevel"
		ewarn "nothing to move around"
	fi
}
